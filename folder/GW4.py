...
def policy(graph, prob):
    #ret = "Policy: \n"
    ret = ""
    jumps = ""
    for v in range(graph['size']):
        if v % graph['width'] == 0: ret += "\n"
        policy = single_policy(graph, v, prob)
        ret += policy
        #if "JUMP" in policy:
        #    ret += policy[0]
...
def single_policy(graph, vertex, problem):
    ...
    for d in dirs:
        if (vertex, d) in graph['props']['eprops'] and 'rwd' in graph['props']['eprops'][(vertex, d)]:
            costs.append(1)
            rwds.append(graph['props']['eprops'][(vertex, d)]['rwd'])
        else:
            ret = BFS(graph, d)
            if not ret[0]: continue
            rwds.append(max(ret[0])) # problem here for G1
            costs.append(min([ret[1][c] for c in range(len(ret[1])) if ret[0][c] == max(ret[0])]))
    if vertex == 1: print(costs)
    if problem == 1:
        for i in range(len(costs)):
            costs[i] = float(rwds[i]) / float(costs[i])  
    if rwds:
        max_rwd = max(rwds)
    else: max_rwd = -1
    j = 0
    while j < len(costs):
        if costs[j] == None or rwds[j] != max_rwd:
            costs.pop(j)
            dirs.pop(j)
            rwds.pop(j)
            j -= 1
...
def BFS(graph, start):
    ...
    while queue:
        ...
        seen.add(v)
        vertex_is_reward = False
        if 'rwd' in graph['props']['vprops'][v]: 
            rwds.append(graph['props']['vprops'][v]['rwd'])
            vertex_is_reward = True
            costs.append(info[1])
        for v2 in graph['edges'][v]:
            if 'rwd' in graph['props']['vprops'][v2] and not vertex_is_reward:
                rwds.append(graph['props']['vprops'][v2]['rwd'])
                costs.append(info[1] + 1)
            if (v, v2) in graph['props']['eprops'] and 'rwd' in graph['props']['eprops'][(v, v2)]:
                rwds.append(graph['props']['eprops'][(v, v2)]['rwd'])
                costs.append(info[1] + 1)
        if 'rwd' not in graph['props']['vprops'][v]:
            queue.extend([(ve, info[1]+1) for ve in graph['edges'][v]])
    return rwds, costs

def args_translate(args):
    new_args = []
    specials = []
    if len(args) == 1:
        return grfParse(["GG"+args[0]]), 1
    new_args.append("GG" + args[0] + ("W"+args[1] if args[1][0] in "0123456789" else ""))
    implied_rwd = 12
    problem = 1
    for arg in args[1:]:
        if arg[0].upper() == 'R':
            if arg[1] == ":": # R:#
                implied_rwd = int(arg[2:])
            elif ":" in arg: # R#:#
                new_args.append("V" + arg[1:arg.index(":")] + "R" + arg[arg.index(":")+1:])
            else: # R#
                new_args.append("V" + arg[1:] + "R" + str(implied_rwd))
        elif arg[0].upper() == 'B':
            if arg[-1] not in "0123456789": # B#[NSEW]+
                new_args.append("E" + "~" + arg[1:] + "=")
            else:
                new_args.append("V" + arg[1:] + "B")
        elif arg[0].upper() == 'G':
            problem = int(arg[1])
    graph = grfParse(new_args)
    return graph, problem

def output_map(out):
    ret = ""
    mapp = {'L':'V', '>':'W','r':'S','v':'T','7':'E','<':'F','J':'M','^':'N','N':'U','W':'L','E':'R','S':'D'}
    for c in out:
        if c in mapp:
            ret += mapp[c]
        else: ret += c
...
def main():
    graph, problem = args_translate(args)
    ...
    print()
    print(propsStr)
    print(policy(graph, problem))
    #print(output_map(policy(graph, problem)))
    print("======")
